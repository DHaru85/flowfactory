...
class KnowledgeRepository:
    ...
    def get_collection_by_code(self, code: str) -> KnowledgeCollection | None:
        stmt = select(KnowledgeCollection).where(KnowledgeCollection.code == code)
        return self._session.scalar(stmt)

    def list_docs_by_collection(self, collection_id: uuid.UUID) -> list[KnowledgeDoc]:
        stmt = select(KnowledgeDoc).where(KnowledgeDoc.collection_id == collection_id)
        return list(self._session.scalars(stmt).all())

    def list_chunks_by_doc(self, doc_id: uuid.UUID) -> list[KnowledgeChunk]:
        stmt = (
    ...
    def get_storage_object(self, bucket: str, object_key: str) -> FileStorageObject | None:
    ...
    def list_pending_ingestion_jobs(self) -> list[KnowledgeIngestionJob]:
