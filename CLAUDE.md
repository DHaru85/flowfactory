...
- **施工工作流程细则**:
    ...
    5. **更新施工状态**: 修改之后，将涉及模块的施工状态更新为 `施工中`, 并记录到 /docs/construction 中的文档里，测试通过后再改回 `空闲`。用来避免多个施工动作之间产生修改方向的冲突
...
- **多轮施工说明**:
    1. 将每次施工方案中计划分步骤完成的任务里面，尚未完成但是计划要去做的任务文档写入 `/docs/unreached`，这个目录中的文档允许在未完事项完成后被删除，执行过程中被修订。当开始进行事项时，应该移动到plan中。此系列方案文档按照正式方案规范编写。
