...
`StartRunRequest.definition` 可内联 `FlowDefinitionDocument`；为空则从 `agent_flow.definition` 读取。图状态固定三槽：`messages` / `variables` / `metadata`。
...
Run 状态：`pending` / `running` / `interrupted` / `completed` / `failed` / `cancelled`。
