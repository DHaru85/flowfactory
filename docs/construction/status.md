...
| 数据层 / service/persistence | 空闲 | Permission 含 refresh/family/grants；Knowledge 检索已落地 |
| 数据层 / service/cache | 空闲 | JwtCacheStore + GrantCacheStore |
| 数据层 / settings | 空闲 | JWT / LDAP / MinIO / embedding |
| 服务层 / service/runtime | 空闲 | LangGraph + vLLM + HITL + Beat |
| 服务层 / service/celery_app | 空闲 | ingest + LDAP sync 骨架 |
